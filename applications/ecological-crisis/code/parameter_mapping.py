"""Ecological-domain parameter mapping for the ICI framework."""
...
class EcologicalICIParameters:
    ...
    notes: str = ''
def validate_positive(params: EcologicalICIParameters) -> None:
    for name in ('D','C','S','F','W','M'):
        value = getattr(params, name)
        if value <= 0:
            raise ValueError(f'{name} must be positive; got {value}.')
def compute_ici(params: EcologicalICIParameters, k: float=K_NORM, alpha: float=ALPHA, fwm_h: float=FWM_H) -> Dict[str, float]:
